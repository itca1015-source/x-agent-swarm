...
- Separate Draft.js paragraph blocks render with large vertical gaps. For this
  workflow, body text must paste compactly: consecutive text blocks are merged
  into one paragraph with soft `<br>` line breaks.
...
3. Rewrite `article.json`:
   ...
   - `body`: ordered list of blocks:
     - `{"type": "p", "text": "..."}` — one logical text block. Use `\n`
       inside `text` for tight line breaks (e.g. lists, score tables). The
       copy script merges consecutive text blocks with soft line breaks so X
       does not add oversized paragraph spacing.
...
2. Copy Article Text → paste into X's article body (text uses compact soft line breaks).
