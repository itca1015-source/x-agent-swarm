...
def _is_btcmind_config(cfg: dict) -> bool:
    handle = str(cfg.get("hunter_handle") or "").strip().lstrip("@").lower()
    acct = str(cfg.get("x_handle") or cfg.get("account") or "").strip().lstrip("@").lower()
    policy = str(((cfg.get("autonomy_policy") or {}).get("account") or "")).strip().lower()
    return handle in {"btcmind101", "hunter_solvea"} or acct in {"btcmind101", "hunter_solvea"} or policy == "btcmind"


def _generate_quote_draft(cfg: dict, candidate: dict, max_chars: int) -> dict:
    if _is_btcmind_config(cfg):
        import btcmind_home_timeline_scout as _bt_home
        ai_decision = {
            "reason": "high-engagement keyword quote candidate",
            "quote_angle": "Add a BTCMind market-structure, wallet/security, stablecoin/payment, on-chain data, or crypto-AI operator implication.",
        }
        return _bt_home.generate_btcmind_quote(candidate, ai_decision, max_chars=max_chars)
    return _generate.generate_quote_tweet(
        target_handle=(candidate.get("author") or "").lstrip("@"),
        target_post_text=candidate.get("text", ""),
...
def main():
    ...
    send_to_tg = bool(qcfg.get("send_to_telegram", True))
    auto_post  = bool(qcfg.get("auto_post", False))
    ...
    daily_cap = int(qcfg.get("daily_draft_cap") or 0)
    if daily_cap and not args.dry_run:
        drafted_today = _daily_count(seen, handle, "quote_drafts")
        remaining_today = max(0, daily_cap - drafted_today)
        if remaining_today <= 0:
            _log(f"quote_scout daily draft cap reached for {handle}: {drafted_today}/{daily_cap}")
            return
        max_drafts = min(int(max_drafts), remaining_today)
    ...
    try:
        ...
        for c in all_cands:
            ...
            if _entry_already_seen(c, seen, dedup_h):
                continue
            try:
                g = _generate_quote_draft(cfg, c, max_chars)
            except Exception as e:
            ...
            if not qt or len(qt) < 20:
                ...
                continue
            if _chrome.text_repeats_itself(qt):
                _log("    generated QT repeats itself, skip")
                continue
            ...
            entry = {
                ...
                "status":         "posting" if auto_post else "pending",
                "needs_human_approval": not auto_post,
                "queued_at":      _ts(),
                "telegram_message_id": 0,
            }
            safety = _btcmind.check_entry("quote", entry, cfg, require_autonomy=auto_post)
            if safety.get("policy_applied"):
            ...
            if args.dry_run:
                status = "safe" if safety.get("ok") else f"blocked: {safety.get('reason', '')}"
                _log(f"  [dry-run:{status}] @{entry['target']} ({entry['post_likes']} likes) → "
                     f"{qt[:120]}")
                if safety.get("ok"):
                    drafted += 1
                else:
            ...
            if not safety.get("ok"):
                ...
                _log(f"    skip QT: BTCMind autonomy safety gate: {safety.get('reason', '')}")
                continue
            try:
            ...
            if auto_post:
                try:
                    res = _engage.quote_tweet(port, entry["target_url"], entry["reply_text"],
                                              dry_run=False,
                    ...
                    with file_lock("reply_queue", on_wait=_log):
                        ...
                        for i, e in enumerate(cur):
                            if e.get("id") == entry["id"]:
                                ...
                                _save(QUEUE_PATH, cur)
                                break
                    _log(f"    auto-posted QT for @{entry['target']}")
                else:
                    ...
                    with file_lock("reply_queue", on_wait=_log):
                        ...
                        for i, e in enumerate(cur):
                            if e.get("id") == entry["id"]:
                                ...
                                _save(QUEUE_PATH, cur)
                                break
                    _log(f"    auto-post QT FAILED: {entry['error']}")
            elif send_to_tg:
                try:
                    if not tg_token or not tg_chat_id:
                        _log("    TG send skipped: configured Telegram env vars are not set")
                        msg_id = 0
                    else:
                    ...
                    if msg_id:
                        ...
                        _log("    TG send returned no message_id")
                except Exception as e:
                    _log(f"    TG send failed: {e}")
            else:
                _log("    telegram notify disabled for quote_scout")
            seen[c.get("id", "")] = datetime.now().isoformat()
            _increment_daily(seen, handle, "quote_drafts")
            _save(SEEN_PATH, seen)
            drafted += 1
