...
def _publish_route(entry: dict, cfg: dict, ncfg: dict) -> str:
    """Return auto_post, human_review, telegram, or queue_only.

    Hunter Autopilot v2 deliberately does not use Telegram approval. It only
    allows the low-risk quote_take shape to auto-ship; originals and
    counter-takes are reputation-risky enough to require a local review slot.
    """
    policy = cfg.get("hunter_autopilot", {}) or {}
    mode = entry.get("news_mode", "")
    kind = entry.get("kind", "")
    human_modes = set(policy.get("human_review_modes",
                                 ["original_reframe", "counter_take"]))
    auto_modes = set(policy.get("auto_ship_modes", ["quote_take"]))

    if mode in human_modes or kind == "original":
        return "human_review"
    if bool(ncfg.get("auto_post", False)) and kind == "quote" and mode in auto_modes:
        return "auto_post"
    if bool(ncfg.get("send_to_telegram", True)):
        return "telegram"
    return "queue_only"
    ...
    try:
        ...
        for c in all_cands:
            ...
            for mode in picked:
                ...
                route = _publish_route(entry, cfg, ncfg)
                if route == "auto_post":
                    entry["status"] = "posting"
                elif route == "human_review":
                    policy = cfg.get("hunter_autopilot", {}) or {}
                    entry["status"] = policy.get("human_review_status", "human_review")
                    entry["review_reason"] = "hunter_autopilot_v2_human_review"
                if _chrome.text_repeats_itself(entry.get("reply_text", "")):
                    _log(f"  generated {mode} repeats itself, skip")
                    continue
                if args.dry_run:
                ...
                if route == "auto_post":
                    ...
                    if res.get("ok"):
                        ...
                        if res.get("reply_url"):
                            ...
                            entry["reply_url_actual"] = res["url"]
                        with file_lock("reply_queue", on_wait=_log):
                            cur = _load(QUEUE_PATH, [])
                            for i, e in enumerate(cur):
                                if e.get("id") == entry["id"]:
                                    ...
                                    _save(QUEUE_PATH, cur)
                                    break
                        _log(f"    auto-posted {kind} for @{c.get('author','')}")
                    else:
                        ...
                        with file_lock("reply_queue", on_wait=_log):
                            ...
                            for i, e in enumerate(cur):
                                if e.get("id") == entry["id"]:
                                    ...
                                    _save(QUEUE_PATH, cur)
                                    break
                        _log(f"    auto-post {kind} FAILED: {entry['error']}")
                elif route == "telegram":
                    try:
                    ...
                    _log(f"    queued for local human review ({mode}); no Telegram")
                else:
                    _log("    queued only; no Telegram")
