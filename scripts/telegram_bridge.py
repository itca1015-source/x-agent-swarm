...
elif _VARIANT == "hunter101":
    ht = os.environ.get("TELEGRAM_BOT_TOKEN_HUNTER", "")
    hc = os.environ.get("TELEGRAM_CHAT_ID_HUNTER", "")
    if not ht or not hc:
        print("BRIDGE_VARIANT=hunter101 requires TELEGRAM_BOT_TOKEN_HUNTER + "
              "TELEGRAM_CHAT_ID_HUNTER in .env", flush=True)
        sys.exit(1)
    os.environ["TELEGRAM_BOT_TOKEN"] = ht
    os.environ["TELEGRAM_CHAT_ID"]   = hc
elif _VARIANT == "flatkey":
    ft = os.environ.get("TELEGRAM_BOT_TOKEN_FLATKEY", "")
    fc = os.environ.get("TELEGRAM_CHAT_ID_FLATKEY", "")
    if not ft or not fc:
        print("BRIDGE_VARIANT=flatkey requires TELEGRAM_BOT_TOKEN_FLATKEY + "
              "TELEGRAM_CHAT_ID_FLATKEY in .env", flush=True)
        sys.exit(1)
    os.environ["TELEGRAM_BOT_TOKEN"] = ft
    os.environ["TELEGRAM_CHAT_ID"]   = fc
elif _VARIANT == "btcmind":
    bt = os.environ.get("TELEGRAM_BOT_TOKEN_BTCMIND", "")
    bc = os.environ.get("TELEGRAM_CHAT_ID_BTCMIND", "")
    if not bt or not bc:
        print("BRIDGE_VARIANT=btcmind requires TELEGRAM_BOT_TOKEN_BTCMIND + "
              "TELEGRAM_CHAT_ID_BTCMIND in .env", flush=True)
        sys.exit(1)
    os.environ["TELEGRAM_BOT_TOKEN"] = bt
    os.environ["TELEGRAM_CHAT_ID"]   = bc
...
_VARIANT_CONFIGS = {
    ...
    "hunter101": os.path.join(ROOT_DIR, "accounts", "hunterguo101", "engage_config.json"),
    "flatkey": os.path.join(ROOT_DIR, "accounts", "flatkey", "engage_config.json"),
    "btcmind": os.path.join(ROOT_DIR, "accounts", "hunter_solvea", "engage_config.json"),
}
...
def _claim_pending_entry(entry_id: str, msg_id: int) -> Tuple[Optional[dict], str]:
    """Atomically transition a pending entry to posting.

    This prevents concurrent bridge workers from executing the same approve
    callback twice when multiple processes are polling Telegram at once.
    """
    with file_lock("reply_queue", on_wait=_log):
        cur = _load(QUEUE_PATH, [])
        idx = _find_entry(cur, entry_id)
        if idx < 0 and msg_id:
            idx = _find_entry_by_msg_id(cur, msg_id)
        if idx < 0:
            return None, "missing"
        q = cur[idx]
...
def _append_to_library(q: dict):
    ...
    lib.append({
        ...
        "reply_author":        q.get("account") or "GuoHunter95258",
...
def _execute(q: dict, cfg: dict) -> dict:
    ...
    reply_quality_enabled = isinstance(qcfg, dict) and bool(qcfg.get("enabled", False))
    quality_label = str((qcfg or {}).get("account_label") or cfg.get("telegram_label") or cfg.get("hunter_handle") or "Reply").strip()
    if is_btcmind and not bool(cfg.get("x_public_posting_enabled", False)):
        return {"ok": False, "error": "BTCMind X public posting is disabled"}
    if kind in {"reply", "quote", "original"} and _chrome.text_repeats_itself(q.get("reply_text", "")):
        return {"ok": False, "error": "draft_text_repeats_before_compose"}
    if kind == "reply":
        try:
            import btcmind_reply_quality as _bq
            if reply_quality_enabled:
                audit = _bq.apply_audit(q, cfg)
                if audit.get("reply_risk_class") in {"needs_rewrite", "block"}:
                    why = " | ".join((audit.get("reasons") or [])[:3])
                    return {"ok": False, "error": f"{quality_label} reply quality {audit.get('reply_risk_class')}: {why}"}
        except Exception as e:
            return {"ok": False, "error": f"{quality_label} reply quality audit failed: {e}"}
    if q.get("dry_run_approve"):
        return {"ok": False, "error": "dry_run_approve_no_post"}
    if kind == "original":
    ...
    if kind == "repost":
        return _engage.retweet_tweet(port, q["target_url"], dry_run=False)
    # reply (default)
...
def _regenerate(q: dict, cfg: dict) -> dict:
    ...
    if kind == "repost":
        raise ValueError("repost approvals do not have generated text to regenerate")
    # reply (default)
    extra_quality_rules = ""
    quality_feedback = ""
    reply_quality = None
    try:
        ...
        source = str(q.get("source") or "").lower()
        qcfg = cfg.get("reply_quality_audit")
        if isinstance(qcfg, dict) and bool(qcfg.get("enabled", False)):
            reply_quality = _bq
    ...
    if reply_quality and g.get("reply"):
        audit_entry = {**q, "reply_text": g.get("reply", "")}
        audit = reply_quality.apply_audit(audit_entry, cfg)
        g["btcmind_reply_audit"] = audit
        g["reply_risk_class"] = audit.get("reply_risk_class", "")
        g["reply_quality_score"] = audit.get("score", 0)
    return g
...
def _summary_block(q: dict) -> str:
    ...
    if kind == "repost":
        return (f"*@{q.get('target','?')}* — age {q.get('post_age_min','?')}m\n"
                ...
                f"*Action:* plain repost")
    if bool(q.get("inbound_engagement")) or q.get("source") == "inbound_engage":
        account = str(q.get("telegram_label") or q.get("account") or "this account").strip().lstrip("@")
        return (f"*INBOUND ENGAGEMENT — responding to engaged user*\n"
                f"*@{q.get('target','?')}* engaged with {account}\n"
                f"*Engaged user's post/comment:* {(q.get('target_text') or '')[:400]}\n"
                f"*Comment-back draft:* {q.get('reply_text','')}")
    return (f"*@{q.get('target','?')}* — age {q.get('post_age_min','?')}m\n"
            f"*OP:* {(q.get('target_text') or '')[:400]}\n"
            f"*{'QT' if kind == 'quote' else 'Reply'}:* {q.get('reply_text','')}")


def _send_post_result_notification(q: dict, ok: bool, error: str = ""):
    kind = q.get("kind", "reply")
    account = q.get("account") or _VARIANT
    if ok:
        url = q.get("reply_url_actual", "")
        if not url and kind == "repost":
            url = q.get("target_url", "")
        url_line = f"\n{url}" if url else ""
        _tg.send_plain_text(f"✅ @{account} {kind} posted{url_line}")
    else:
        _tg.send_plain_text(f"❌ @{account} {kind} failed\n{error[:300]}")


def _save_queue_entry(entry_id: str, msg_id: int, q: dict):
    # Reload-merge-save under lock so executor work cannot clobber writes from
    # producers that ran while the post was being executed.
    with file_lock("reply_queue", on_wait=_log):
        cur = _load(QUEUE_PATH, [])
...
def _execute_approved_async(entry_id: str, q: dict, entry_cfg: dict,
                            kind: str, msg_id: int, summary: str):
    try:
        res = _execute(q, entry_cfg)
    except Exception as e:
    ...
    _save_queue_entry(entry_id, msg_id, q)
    if msg_id:
        _tg.edit_card(msg_id, summary, footer)
    _send_post_result_notification(
        q,
        bool(res.get("ok")),
        res.get("error", q.get("error", "")),
    )


def _config_for_entry(q: dict, fallback: dict) -> dict:
    """Resolve the account config for a queued card.

    This lets one Telegram bot token serve multiple account approval cards.
    Telegram only allows one getUpdates poller per bot token, so when BTCMind
    reuses the Flatkey approval bot the active Flatkey bridge must still post
    BTCMind cards through BTCMind's own X config.
    """
    account = str(q.get("account") or "").strip().lstrip("@").lower()
    source = str(q.get("source") or "").strip().lower()
    safety_policy = str(q.get("safety_policy") or "").strip().lower()
    variant = ""
    if account in {"btcmind101", "hunter_solvea"} or source.startswith("btcmind") or safety_policy.startswith("btcmind"):
        variant = "btcmind"
    elif account == "flatkey":
        variant = "flatkey"
    elif account in {"voc_ai", "shulex"}:
        variant = "voc_ai"
    elif account in {"hunterguo101", "hunter101"}:
        variant = "hunter101"
    elif account in {"guohunter95258", "hunter"}:
        variant = "hunter"
    path = _VARIANT_CONFIGS.get(variant) if variant else ""
    if not path:
        return fallback
    cfg = _load(path, {})
    if cfg:
        _log(f"resolved callback config: account={account or '?'} source={source or '?'} variant={variant}")
        return cfg
    _log(f"callback config fallback: variant={variant} path missing or invalid")
    return fallback
...
def _handle_callback(cq: dict, cfg: dict, state: dict):
    ...
    cb_id = cq.get("id", "")
    callback_msg_id = int((((cq.get("message") or {}).get("message_id")) or 0))
    ...
    q = queue[idx]
    if callback_msg_id and not int(q.get("telegram_message_id", 0) or 0):
        q["telegram_message_id"] = callback_msg_id

    if q.get("status") != "pending":
        _tg.answer_callback(cb_id, f"already {q.get('status')}")
        return

    entry_cfg = _config_for_entry(q, cfg)
    kind = q.get("kind", "reply")
    msg_id = q.get("telegram_message_id", 0)
    summary = _summary_block(q)

    if action == "approve":
        q_claimed, claim = _claim_pending_entry(entry_id, msg_id)
        if claim == "missing":
            _log(f"approve miss after claim: entry_id={entry_id} msg_id={msg_id}")
            _tg.answer_callback(cb_id, "entry not found")
            return
        if claim != "claimed":
            _tg.answer_callback(cb_id, f"already {claim}")
            return
        q = q_claimed
        if callback_msg_id and not int(q.get("telegram_message_id", 0) or 0):
            q["telegram_message_id"] = callback_msg_id
        ...
        if msg_id:
            _tg.edit_card(msg_id, summary, "⏳ *posting...* do not tap again")
        threading.Thread(
            target=_execute_approved_async,
            args=(entry_id, q, entry_cfg, kind, msg_id, summary),
            daemon=True,
        ).start()
        _log(f"approve accepted; posting worker started for {kind} ({entry_id})")
        return
        ...
        if msg_id:
            _tg.edit_card(msg_id, summary, "⏳ *regenerating...*")
        try:
            g = _regenerate(q, entry_cfg)
            q["reply_text"]  = g.get("reply", "")
            q["op_summary"]  = g.get("op_summary", "")
            q["reply_angle"] = g.get("reply_angle", "")
            if g.get("btcmind_reply_audit"):
                q["btcmind_reply_audit"] = g["btcmind_reply_audit"]
                q["reply_risk_class"] = g.get("reply_risk_class", "")
                q["reply_quality_score"] = g.get("reply_quality_score", 0)
...
def _handle_message(msg: dict, cfg: dict, state: dict):
    ...
    if g.get("btcmind_reply_audit"):
        new_entry["btcmind_reply_audit"] = g["btcmind_reply_audit"]
        new_entry["reply_risk_class"] = g.get("reply_risk_class", "")
        new_entry["reply_quality_score"] = g.get("reply_quality_score", 0)
