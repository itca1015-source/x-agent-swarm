...
def generate_engaged_reply(target_handle: str, target_post_text: str,
    ...
    display_handle:    optional live X handle used in the prompt; defaults to hunter_handle
    ...
    extra_quality_rules: optional account-specific rules appended to the prompt
    quality_feedback: optional local audit feedback when regenerating a draft
    ...
    if not chosen:
        return {
            "reply": _fallback_reply(target_post_text, max_reply_chars),
            "op_summary": "",
            "reply_angle": "fallback_reply",
        }
    ...
    prompt_handle = (display_handle or hunter_handle).lstrip("@")
    playbook_block = f"Your voice (you are operating @{prompt_handle}):\n{hunter_playbook}\n\n" if has_real_playbook else ""
    ...
    quality_rules_block = (
        f"\n\nADDITIONAL QUALITY RULES:\n{extra_quality_rules.strip()}\n"
        if extra_quality_rules and extra_quality_rules.strip() else ""
    )
    quality_feedback_block = (
        f"\n\nLOCAL AUDIT FEEDBACK TO FIX:\n{quality_feedback.strip()}\n"
        if quality_feedback and quality_feedback.strip() else ""
    )

    prompt = f"""{playbook_block}You are writing a reply on X to a post in this account's niche. Below are reply
...
{tone_brief}{quality_rules_block}{quality_feedback_block}
...
- Use at least two concrete anchors from the OP when possible: a named product,
  company, mechanism, number, quoted claim, incident, or exact constraint.
  If you cannot name the anchors, keep the reply plain and human instead of
  adding an abstract thesis.
- Match the OP's register. If the post is casual, joking, short, promotional,
  or a non-serious one-liner, keep the reply casual and light. Do NOT turn it
  into polished consultant analysis or a technical thesis.
- Do NOT force your account niche onto broad news, cybersecurity, politics, or
  drama. If the OP does not explicitly discuss agents, routing, token cost,
  permissions, model behavior, context, or tools, do not introduce those as the
  diagnosis.
- Avoid synthetic consultant contrast shapes: "not a comms problem, that's a
  permissions problem", "not X, the real issue is Y", "the real bottleneck",
  and "who actually decides what X optimizes for" unless the OP itself gives
  concrete evidence for that exact contrast.
